...
VertexPositionInputs GetVertexPositionInputs2(float3 positionWS)
{
...
Varyings LitPassVertex(Attributes input, uint instanceID : SV_InstanceID)
{
	...
	if(dot(vertex.normal, float3(0, 1, 0))< 0.9)
	{
		output.color = _SideColor;
	}
	else
	{
		output.color = _UpColor;
	}

	float3 position = unitData.position.xyz + vertex.position.xyz * float3(_UnitSize, _LevelHeight, _UnitSize);
	float3 positionWS = position;
	input.texcoord = float4(vertex.uv, 0, 0);
	vertex.normal.xz *= _NormalStrength;
	input.normalOS = vertex.normal;

    VertexPositionInputs vertexInput = GetVertexPositionInputs2(positionWS);
